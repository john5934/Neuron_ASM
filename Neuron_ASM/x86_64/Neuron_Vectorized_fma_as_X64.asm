;-------------------------------------------------------------------------------
; Neuron_Vectorized_fma_as_X64.asm                   REQUIRES FMA SUPPORT IN CPU
;-------------------------------------------------------------------------------
;
...
; are identical.
;
; This version uses fused multiply-add instructions, so it can only run on Xeon
; and compatible x64 machines. There is a version of this routine for non-Xeon
; that does not use fused multiply-add instructions: see
;                                                 Neuron_Vectorized_as_X64.asm .
; REQUIRES:
;
;   A x86_64 CPU such as Xeon that supports fused multiply-add x64 instructions.
;   Look for 'fma' flag when running lscpu in a terminal window.
;
;   InputCount > 0
;
...
;
; HISTORY:
;    21Dec2023 From Neuron(), Neuron_Vectorized_as_X64.asm, and
;              RowTimesMatrix_ColumnMajor_Even_fma_as.asm.
;    21Dec2023 Passed Neuron_Vectorized_as_Test() OK on Xeon.
;-------------------------------------------------------------------------------

%include "platform_nasm.inc"

cpu all ; Need this for the fma instructions.


global Neuron_Vectorized_fma_as

;-------------------------------------------------------------------------------
;   // OUT: Neuron output value in xmm0 ready for an activation function.
; f64
; Neuron_Vectorized_fma_as(
;     f64* Inputs,
;              // Input values fed to the neuron.
;              //
;     f64* Weights,
...
;////////
    ...
    movupd xmm15, [rsi+80]
                    ...
                    ; Multiply-add 6 pairs of weights with 6 pairs of inputs.

                    ; Fused multiply-add packed double-precision floating-point
                    ; values. Multiply 2nd and 3rd parameters and add to xmm1.
    ...
    movupd xmm11, [rsi+112]
                    ...
                    ; Multiply-add 2 pairs of weights with 2 pairs of inputs.

                    ; Fused multiply-add packed double-precision floating-point
                    ; values. Multiply 2nd and 3rd parameters and add to xmm1.
    vfmadd231pd xmm1, xmm8, xmm10
    vfmadd231pd xmm1, xmm9, xmm11
...
;//////////
    ...
    movupd xmm15, [rsi+96]
                    ...
                    ; Multiply-add 7 pairs of weights with 7 pairs of inputs.

                    ; Fused multiply-add packed double-precision floating-point
                    ; values. Multiply 2nd and 3rd parameters and add to xmm1.
    vfmadd231pd xmm1, xmm2, xmm9
    vfmadd231pd xmm1, xmm3, xmm10
    vfmadd231pd xmm1, xmm4, xmm11
    vfmadd231pd xmm1, xmm5, xmm12
    vfmadd231pd xmm1, xmm6, xmm13
    vfmadd231pd xmm1, xmm7, xmm14
    vfmadd231pd xmm1, xmm8, xmm15
...
;//////////
    ...
    movupd xmm13, [rsi+64]
                    ...
                    ; Multiply-add 5 pairs of weights with 5 pairs of inputs.
...
;/////////
    ...
    movupd xmm12, [rsi+48]
                    ...
                    ; Multiply-add 4 pairs of weights with 4 pairs of inputs.
...
;/////////
    ...
    movupd xmm11, [rsi+32]
                    ...
                    ; Multiply-add 3 pairs of weights with 3 pairs of inputs.
...
;/////////
    ...
    movupd xmm9, [rsi]
                    ...
                    ; Multiply-add 1 pair of weights with 1 pair of inputs.
